...
\paragraph*{(20)} 
Derive the \href{https://uidaho.pressbooks.pub/nuclearengineering/chapter/radioactive-decay/}{decay equation} for a three member chain (A, B, C), where the third member is a stable nuclide. At what time is the activity of B maximized? Based on the three member decay chain, when will secular equilibrium be reached if $\lambda_A \ll \lambda_B$ or if $\lambda_B \ll \lambda_A$ Discuss why secular equilibrium is useful.
