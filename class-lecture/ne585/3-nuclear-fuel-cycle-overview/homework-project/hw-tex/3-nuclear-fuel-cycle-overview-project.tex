...
\chead{\scriptsize \textit{Project 3 -- Front end of the nuclear fuel cycle}}
...
\begin{titlepage}
    \title{
        ...
        Project 3 -- Front end of the nuclear fuel cycle\\
    }
    \author{
        Name
        \\ \\ \\
...
\section*{Preface}
Refer to these references to start for aqueous reprocessing --
\begin{enumerate}
    \item\href{https://piazza.com/class_profile/get_resource/mcf5xaxnhxh5qs/mcf5yw70pmh7c6}{Countercurrent equilibrium extraction -- Benedict book excerpt}
    \item\href{https://piazza.com/class_profile/get_resource/ll13mxo3tb15l/ll2oqtgzlro4jr}{Nuclear fuel reprocessing report}
    \item\href{https://piazza.com/class_profile/get_resource/ll13mxo3tb15l/ll2oqumeord4pf}{Principles of stagewise separation process calculations report}
    \item\href{https://youtu.be/HGwHLaPhw30}{Liquid-liquid extraction example video problem I}
    \item\href{https://youtu.be/vK8XGYwnZv4}{Liquid-liquid extraction example video problem II}
\end{enumerate}
...
\textit{Required for full credit -- } In problems 3 -- 8, please address how the solution will affect the engineering design of a commercial scale PUREX facility. 

\newpage

\section{Volume flow -- 10)}
...
\section{SWU -- 10)}
Plot $\frac{SWU}{P}$ and $\frac{SWU}{F}$ as a function of $x_P$. There is an interesting implication(s) of this curve, which we talked about, in terms of the JPCOA. What is it? Note that medical isotopes require $x_P = 0.20$.
...
\section{Extract concentration -- (20)}
...
\section{Decontamination factor -- (20)}
...
\section{Maximum decontamination factor -- (25)}
\begin{enumerate}
    \item Consider a multistage extraction system with two extractable components. What is the theoretical maximum for the decontamination factor? (\textit{Assume $y_0 = 0$}.)

    \begin{itemize}
        \item$\beta_A > 1, \quad \beta_B > 1$
        \item$\beta_A > 1, \quad \beta_B < 1$
        \item$\beta_A < 1, \quad \beta_B > 1$
        \item$\beta_A < 1, \quad \beta_B < 1$
    \end{itemize}

    \item Then, if the extraction factor for U is 5 and for Tc is 0.01. What is the decontamination factor? What does it physically mean?
\end{enumerate}
...
\section{Uranium tails -- (50)}
How are uranium tails managed? Critically discuss this issue.
...
\section{BONUS -- Enrichment -- (50)}
What would be the speed and radius of a gas centrifuge that would make it equivalent to gas diffusion? What are the dimensions of the gas centrifuge? Which is better? Compute the separation factor and \textit{solve} for velocity and radius.
...
\section{BONUS -- Cost analysis -- (50)}
Taking the cost equation, what is the driver or limited factor for cost? What needs to happen for an enrichment plant to be profitable? What challenges are there? Feel free to make graph(s).

\begin{equation}
    c_P = [V_P - V_F - (x_P-x_F) \frac{V_F-V_W}{x_f-x_w}] c_S + (\frac{x_P-x_W}{x_F-x_w}) c_F
\end{equation}

\begin{enumerate}
    \item[]$c_P$ = enriched U price
    \item[]$c_S$ = SWU price
    \item[]$c_F$ = natural uranium price
    \item[]$V_i = V(x_i)$; i.e., value function
\end{enumerate}
